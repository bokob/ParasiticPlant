...
public class NewArrow : MonoBehaviour
{
    ...
    [SerializeField] bool _isHit = false;                                       // 화살이 어디에 맞았는지 여부
    ...
    void CheckCollision()
    {
        ...
        //    // 충돌한 지점 계산
        //    Vector2 collisionPoint = hit.ClosestPoint(transform.position + transform.right * 0.475f);
        //    _stickPosition = collisionPoint;

        //    // 충돌 지점에 표시를 위해 디버그 드로우
        //    Debug.Log($"Start: {transform.position}, End: {collisionPoint}");
        //    Debug.DrawLine(transform.position, collisionPoint, Color.red, 1f);
